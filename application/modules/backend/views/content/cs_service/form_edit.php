      ...
      <div class="panel panel-default">
        <div class="panel-body">
          <table class="table table-bordered">
            ...
            <tr>
              <td colspan="6">
                <?php if ($cek_service = $this->model->cek_service($jarak_tempuh,$waktu_tempuh)): ?>
                  ...
                  <hr>
                  <?=select_service_edit($cek_service->id_service,$id_service);?>

                  <div id="service_perbaikan" class="row">
                    <?=j_perbaikan_cs_edit($id_service,$id_trans_cs_service)?>
                  </div>
            ...
            <div class="form-group">
              <input type="text" name="id_service2" id="id_service2" value="<?=$id_service?>">
              <input type="text" name="check" id="chkd">
              <input type="text" name="id_trans_kendaraan" id="id_trans_kendaraan" value="<?=$id_trans_kendaraan?>">
              <input type="text" name="jarak_tempuh" id="jarak_tempuh" value="<?=$jarak_tempuh?>">
              <input type="text" name="waktu_tempuh" id="waktu_tempuh" value="<?=$waktu_tempuh?>">
            </div>
...
$(document).on('change', '.form-check-input', function(e){
  ...
  $(document).ready(function(){
        ...
        $('#chkd').val(selected);
          ...
          $(document).on('change', '#id_service', function(e){
            $('#chkd').val('');
            var id_service = $("#id_service option:selected").attr("value");
            $.ajax({
              url : "<?=base_url()?>backend/cs_service/service_perbaikan_edit/<?=$id_trans_cs_service?>/<?=$id_service?>",
              method : "POST",
              data : {id: id_service},
              dataType : "json",
