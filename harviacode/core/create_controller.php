...
$string .="\n\n function index(){
    ...
    \$this->layouts->view($load_view'content/$c_url/$v_list',array(),\$data);
  }";
}else {
$string .="\n\n function index(){
    \$this->layouts->set_title('".ucwords($title)."');
    \$this->layouts->view($load_view'content/$c_url/$v_list');
  }
...
$string .= "\n\t\t\t\t\t\t\t\t\t);";
$string .="\n\t\t\t\t \$this->layouts->view($load_view'content/$c_url/$v_read',array(),\$data);
//MODAL DETAIL
//\$this->layouts->view($load_view'content/$c_url/$v_read',array(),\$data,false);
    }else {
        \$this->error_404();
    }
  }";
...
$string .="     \n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t);
      ...
      \$data = array('layout_title' => '".ucwords($title)."',
                      'button'=>'tambah',
                      'aksi' =>site_url($load_view'$c_url/tambah/aksi'),";
foreach ($all as $row) {
    $string .= "\n\t\t\t\t\t\t\t\t\t\t\t'" . $row['column_name'] . "' => set_value('" . $row['column_name'] . "'),";
}
$string .="\n\t\t\t\t\t\t\t\t\t\t\t);";
$string .="\n\t\t\t \$this->layouts->view($load_view'content/$c_url/form',array(),\$data);
  //MODAL TAMBAH
  //\$this->layouts->view($load_view'content/$c_url/form',array(),\$data,false);
     }
  }";
...
$string .="     \n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t);
      ...
      if(\$row=\$this->model->get_where(\$id)){
        ...
        \$data = array('layout_title' => '".ucwords($title)."',
                      'button'=>'edit',
                      'aksi' =>site_url($load_view'$c_url/edit/'.\$id.'/aksi'),";
foreach ($all as $row) {
